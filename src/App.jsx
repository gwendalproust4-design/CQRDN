      ...
      <div className="flex flex-col md:flex-row justify-between items-end border-b border-survie-grey/30 pb-6 mb-12 gap-6">
        ...
        <div className="flex flex-col gap-2 w-full md:w-auto">
          ...
          <div className="flex gap-4">
            <button onClick={() => window.open(DISCORD_LINK, '_blank')} className="flex-1 md:flex-none text-xs font-bold text-survie-grey border border-survie-grey px-6 py-3 hover:bg-survie-grey hover:text-black uppercase tracking-widest transition-all">
              Discord
            </button>
            <button onClick={onGoToInventory} className="flex-1 md:flex-none text-xs font-bold text-black bg-survie-khaki border border-survie-khaki px-6 py-3 hover:bg-white hover:border-white uppercase tracking-widest transition-all shadow-[0_0_15px_rgba(138,154,91,0.4)]">
              Inventaire →
            </button>
          </div>
        </div>
      ...
      <div className="space-y-24">
          ...
          <div className="order-2 md:order-1">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="font-display text-xl text-white uppercase tracking-widest">
                <HackerText text="OBJECTIF PRINCIPAL" />
              </h2>
            </div>
            <p className="text-gray-400 leading-relaxed mb-4 border-l-2 border-survie-grey/20 pl-4">
              Dans quelques semaines, l'unité Alpha quittera la civilisation. <strong className="text-white">Zéro confort. Zéro filet de sécurité.</strong>
            </p>
            <p className="text-gray-400 leading-relaxed pl-4">
              L'objectif est simple : Tenir 30 jours en autonomie totale. Nous documenterons chaque victoire et chaque échec. Sans filtre. Ce n'est pas de la télé-réalité, c'est de la survie brute.
            </p>
          </div>
            ...
            >
              {isAdmin && (
                <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 z-50 pointer-events-auto">
                  <EditableImage
                    src="" alt="" className="absolute inset-0 opacity-0"
                    isAdmin={true} onUpload={(file) => updateImage('aventure_intro', file)}
                  />
                  <span className="bg-black text-white text-xs px-2 py-1">CHANGER SOURCE</span>
                </div>
              )}
            </DroneImage>
          </div>
        ...
        <div className="grid md:grid-cols-2 gap-12 items-center">
          <div className="h-64 w-full relative">
            <DroneImage
              src={images['aventure_totem'] || "https://images.unsplash.com/photo-1533552222634-934c9973273e?auto=format&fit=crop&q=80"}
              alt="Totem"
              className="w-full h-full"
            >
              {isAdmin && (
                <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 z-50 pointer-events-auto">
                  <EditableImage
                    src="" alt="" className="absolute inset-0 opacity-0"
                    isAdmin={true} onUpload={(file) => updateImage('aventure_totem', file)}
                  />
                  <span className="bg-black text-white text-xs px-2 py-1">CHANGER SOURCE</span>
                </div>
              )}
          ...
          <div>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="font-display text-xl text-white uppercase tracking-widest">
                <HackerText text="OPÉRATION TOTEM" />
              </h2>
            </div>
            <p className="text-gray-400 leading-relaxed mb-4">
              Au-delà de la survie physique, c'est une quête de sens. Que laisse-t-on derrière soi ?
            </p>
            <p className="text-gray-400 leading-relaxed">
              Sur l'île, nous bâtirons un <strong>Totem</strong>. Chaque contributeur verra son nom gravé dans le bois. Ce Totem restera sur l'île comme témoin de notre passage et de votre soutien. <span className="text-survie-khaki">Votre nom, gravé dans l'histoire.</span>
            </p>
          </div>
        </div>
        ...
        <div className="border border-survie-khaki/30 bg-survie-card/20 p-8 relative">
          ...
          <div className="absolute bottom-0 right-0 w-4 h-4 border-b border-r border-survie-khaki"></div>

          <h2 className="font-display text-2xl text-center text-white mb-8 uppercase">
            <span className="text-survie-khaki">VOTRE RÔLE :</span> <HackerText text="LE 3ÈME SURVIVANT" />
          </h2>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-center">
            <div className="group p-6 border border-survie-grey/20 hover:border-survie-khaki/60 transition-all bg-black/40 hover:bg-survie-khaki/5">
              <div className="text-3xl mb-2">📦</div>
              <h3 className="font-display text-white text-lg uppercase mb-2">Logistique</h3>
              <p className="text-xs text-gray-500 font-mono">Chaque achat finance directement nos rations et notre équipement.</p>
            </div>
            <div className="group p-6 border border-survie-grey/20 hover:border-survie-khaki/60 transition-all bg-black/40 hover:bg-survie-khaki/5">
              <div className="text-3xl mb-2">💊</div>
              <h3 className="font-display text-white text-lg uppercase mb-2">Sécurité</h3>
              <p className="text-xs text-gray-500 font-mono">Vos dons payent la trousse de secours et le téléphone satellite.</p>
            </div>
            <div className="group p-6 border border-survie-grey/20 hover:border-survie-khaki/60 transition-all bg-black/40 hover:bg-survie-khaki/5">
              <div className="text-3xl mb-2">📡</div>
              <h3 className="font-display text-white text-lg uppercase mb-2">Intel</h3>
              <p className="text-xs text-gray-500 font-mono">Sur Discord, vous votez pour les décisions cruciales de l'aventure.</p>
            </div>
          </div>
        </div>

        <div className="text-center pt-8 pb-4 opacity-80">
          <p className="font-display text-lg md:text-2xl text-white uppercase italic tracking-wide">
            "<HackerText text="NE SOYEZ PAS SPECTATEURS." /> <br />
            <span className="text-survie-khaki font-bold"><HackerText text="SOYEZ ACTEURS DE NOTRE SURVIE." /></span>"
          </p>
        </div>
      </div>
    </div>
  ...
  return (
    ...
    >
      ...
      <div className={`
        ...
        <p className="text-gray-300 text-xs font-light leading-relaxed">{member.description}</p>
      </div>
    </div>
  );
}
      ...
      <div className="relative z-10 w-full max-w-6xl px-4 pt-24 md:pt-32 text-center flex flex-col flex-grow pointer-events-none">
        ...
        {editingMember && (
          <div className="pointer-events-auto">
            <MemberModal member={editingMember} onClose={() => setEditingMember(null)} onSuccess={updateMember} />
          </div>
        )}
          ...
          <div className="w-1/3 flex justify-start">
            {members[0] && (
              <MemberCard
                ...
                onDelete={async (id) => { if (window.confirm('Supprimer?')) { await supabase.from('team_members').delete().eq('id', id); fetchMembers(); } }}
              />
            )}
          </div>
...
// --- MODAL EDITION/CRÉATION ITEM (ADMIN) ---
...
const CarteObjet = ({ item, onDonationSuccess, onImageUpdate, onDelete, onEdit, isAdmin, user, openAuth }) => {
  ...
  const confirmerDon = () => {
    onDonationSuccess(item.id, parseFloat(montantDon));
    setStep(0);
    setMontantDon("");
  };
  ...
  const handleImageUpload = async (event) => {
    ...
    try {
      ...
      if (!file) return;
      const fileExt = file.name.split('.').pop();
      const fileName = `${item.id}-${Math.random().toString(36).substring(2)}.${fileExt}`;
      const { error: uploadError } = await supabase.storage.from('images-objets').upload(fileName, file);
      if (uploadError) throw uploadError;
      const { data: { publicUrl } } = supabase.storage.from('images-objets').getPublicUrl(fileName);
    ...
    <div className="h-full w-full group relative bg-survie-card border border-survie-grey/30 p-0 overflow-hidden flex flex-col select-none bg-opacity-100 hover:border-survie-khaki/50 transition-colors">
      ...
      <div className="h-32 md:h-64 w-full bg-[#0f0f0f] relative flex items-center justify-center overflow-hidden shrink-0 group/image">
        {item.image_url ? (
          ...
          <div className="text-survie-grey border-2 border-dashed border-survie-grey/20 p-8 rounded"><span className="font-display text-4xl opacity-20">IMG</span></div>
        )}
        ...
        {isAdmin && (
          <div
            className="absolute inset-0 bg-black/60 flex items-center justify-center opacity-0 group-hover/image:opacity-100 transition-opacity duration-300 pointer-events-auto cursor-pointer"
            onClick={() => fileInputRef.current?.click()}
          >
            <span className="text-white text-xs uppercase font-bold tracking-widest border border-white px-3 py-1 hover:bg-white hover:text-black transition-colors">
              {uploading ? '...' : '📷 Admin'}
            </span>
      ...
      <div className="p-3 md:p-6 flex flex-col flex-grow bg-survie-card">
        ...
        <p className={`text-gray-400 text-[9px] md:text-xs mb-2 md:mb-4 font-light border-l-2 border-survie-grey/30 pl-2 md:pl-4 flex-grow leading-tight ${step > 0 ? 'hidden' : 'line-clamp-3'}`}>
          {item.description}
        </p>

        <div className="mt-auto">
          <div className="flex justify-between text-[9px] font-bold text-survie-khaki mb-1 md:mb-2 uppercase tracking-[0.2em]">
            <span>{item.cagnotte}€</span>
            <span>{Math.round(pourcentage)}%</span>
          </div>
          <div className="w-full h-0.5 md:h-1 bg-survie-grey/30 mb-2 md:mb-4 relative">
          ...
          {isCompleted ? (
            ...
            <button onClick={handleContributeClick} className="w-full py-2 md:py-3 bg-transparent border border-survie-khaki text-survie-khaki hover:bg-survie-khaki hover:text-black font-display uppercase tracking-widest text-[9px] md:text-xs transition-all duration-300 active:scale-95 touch-manipulation relative z-10">
              Contribuer {user ? '' : '(Connexion)'}
            </button>
          ) : step === 1 ? (
            ...
            <div className="bg-green-900/20 p-2 rounded border border-green-500/30 animate-fade-in-up relative z-10">
              <p className="text-[9px] md:text-xs text-white text-center mb-2">Virement PayPal effectué ?</p>
              <button onClick={confirmerDon} className="w-full py-2 bg-green-600 hover:bg-green-500 text-white font-bold uppercase text-[9px] md:text-xs tracking-widest mb-2">Je confirme</button>
              <button onClick={() => setStep(1)} className="text-[9px] text-survie-grey w-full text-center hover:text-white uppercase">Non</button>
            </div>
          )}
        </div>
      </div>
...
export default function App() {
  ...
  const handleTouchMove = (e) => {
    ...
    targetRotation.current += deltaX * 0.5;
      ...
      <header className="fixed w-full z-40 bg-survie-bg/80 backdrop-blur-md border-b border-survie-grey/20 pointer-events-auto">
        <div className="max-w-[95vw] mx-auto px-4 py-3 flex justify-between items-center">
          ...
          <div className="flex items-center gap-3 md:gap-4">
            ...
            <nav className="flex md:hidden gap-2 mr-1">
              <button onClick={() => setView('aventure')} className={`text-[10px] uppercase tracking-widest ${view === 'aventure' ? 'text-survie-khaki font-bold' : 'text-gray-500'}`}>QG</button>
              <button onClick={() => setView('home')} className={`text-[10px] uppercase tracking-widest ${view === 'home' ? 'text-survie-khaki font-bold' : 'text-gray-500'}`}>Arsenal</button>
              <button onClick={() => setView('about')} className={`text-[10px] uppercase tracking-widest ${view === 'about' ? 'text-survie-khaki font-bold' : 'text-gray-500'}`}>Infos</button>
            </nav>
                ...
                <div ref={carouselRef} className="carousel-3d">
                  {items.map((item, index) => (
                    <div key={item.id} className="card-container-3d" style={{ transform: `rotateY(${index * anglePerItem}deg) translateZ(${radius}px)` }}>
                      <CarteObjet
                        item={item}
                        onDonationSuccess={handleDonation}
                        onImageUpdate={fetchItems}
